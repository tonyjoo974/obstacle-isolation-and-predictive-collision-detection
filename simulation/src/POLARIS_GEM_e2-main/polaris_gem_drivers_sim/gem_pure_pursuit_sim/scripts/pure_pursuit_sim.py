...
# costume added
from predictTrack import predict_track
...
class PurePursuit(object):
    ...
    def start_pp(self):
        ...
        pt = predict_track()
        pt.updateMotionPara(self.ackermann_msg.steering_angle / 2, self.ackermann_msg.speed, acceleration_default)
        ...
        while not rospy.is_shutdown():
            ...
            # controller for braking the car if its not safe
            if not safety:
                end_time = time.time()
                self.total_time = min(end_time-start_time, self.total_time)
                if unsafe_time == -0.01:
                    text = 'not safe'
                    print(text)
            ...
            if not brake:
                ...
                if drive_speed > 0:
                    text = 'Braking'
                    drive_speed = max(drive_speed - 0.2, 0)
                else:
                    drive_speed = 0
            
            # implement constant pure pursuit controller
            self.ackermann_msg.speed          = drive_speed
