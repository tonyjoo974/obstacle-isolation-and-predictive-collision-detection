...
class SafetyDetector:
    ...
    def checkSafety(self, pedestrianPosition):
        ...
        theta = yaw - 1.57
        self.transformation = np.array([[self.__resolution*np.cos(theta), -1*self.__resolution*np.sin(theta), 0.0],
                                        [self.__resolution*np.sin(theta), self.__resolution*np.cos(theta),  0.0],
                                        [0,          0,           1]])
        # if ped distance is 0 - no pedestrian was detected
        if pedestrianPosition == 0:
            return True, 0, None

        world_car_pos = np.array([0, 0])
    ...
    def apply_transformation(self, ped_pos_img, car_pos_world, theta, scale):
        ...
        self.transformation = np.array([[scale*np.cos(theta), -1*scale*np.sin(theta), car_pos_world[0]],
                                  [scale*np.sin(theta), scale*np.cos(theta),  car_pos_world[1]],
                                  [0,          0,           1]])

        ped_pos_world = np.matmul(self.transformation, pos)
